...
const ys = [new Value(1.0), new Value(-1.0), 
...
while (i < 10) {
  ...
  const loss_list: Value[] = ys.map((_, i) => (ys[i].subtract(ypred[i])).pow(2));
  ...
  // console.log(loss)
  n.params().slice(0,10).forEach((p) => console.log('p before grad: ', p.data, p.grad))
  loss.backward();
  n.params().slice(0,10).forEach((p) => console.log('params: ', p.data, p.grad))
  ...
  n.params().forEach((p) => {
   ...
   n.params().slice(0,10).forEach((p) => console.log('p after backprop: ',p.data, p.grad))
  console.log('-------------------------------------------------------------------')
